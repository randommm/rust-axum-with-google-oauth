...
pub async fn oauth_return(
    ...
    let oauth2_state_storage =
        ...
        .await?.ok_or("OAuth: csrf_state not found on DB")?;
    let pkce_code_verifier = oauth2_state_storage.get("pkce_code_verifier").ok_or("OAuth: pkce_code_verifier not found on DB")?.as_str().ok_or("OAuth: pkce_code_verifier is not a str on DB")?.to_owned();
    let return_url = oauth2_state_storage.get("return_url").ok_or("OAuth: return_url not found on DB")?.as_str().ok_or("OAuth: return_url is not a str on DB")?.to_owned();
